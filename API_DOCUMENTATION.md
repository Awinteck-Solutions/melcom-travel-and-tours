...
The Melcom Travel & Tours API is a comprehensive flight booking system
integrated with the GOL (Golibe) API platform. It provides real-time flight
search, deals, booking management, and utility services.
...
Currently, the API uses server-side GOL API credentials. No client
authentication is required for public endpoints.
...
You can import the OpenAPI specification (`openapi.yaml`) into Postman to
generate a collection with all endpoints and examples.
...
_This documentation is for the Melcom Travel & Tours Flight API v1.0.0_
